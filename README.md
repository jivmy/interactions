...
The chrome is meant to feel like an Apple design prototype: SF Pro, paper/metal materials, interruptible springs, and Core Haptics that mean something. In-track switches ease a short distance; the thumb bar is a page control. Catalog cards carry a track-color index mark so A–E keep a visual beat. Reduce Motion is honored on chrome, not physics. Metal and fluids pause off-screen. VoiceOver can adjust the zipper, the dial, and in-track neighbors. Coaching is a single word.
